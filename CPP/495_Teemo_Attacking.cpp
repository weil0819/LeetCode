...
public:
	/**
	* Idea: we need two points to record the starting and ending points
	* There are two cases: 1) start > last end, directly add duration
	* 2) start <= last end, duration = new end - last end
	* 
	*/
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        ...
        for(int i = 1; i < timeSeries.size(); i++){
        	if(timeSeries[i] > end){
        		...
        		output += timeSeries[i] + duration -1 - end;
        		start = timeSeries[i];
        		end = timeSeries[i] + duration -1;
        	}
        }
...
int main(){
	...
	int duration = 2;
	cout<<sn.findPoisonedDuration(timeSeries, duration)<<endl;
	return 0;
}