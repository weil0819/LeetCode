...
public:
    /**
    * Idea: we can use map<int, int> to record <start, minIndex>
    * and then traverse each interval's end to decide whether there is a "Right Interval"
    * 
    */
    vector<int> findRightInterval(vector<Interval>& intervals) {
        ...
        map<int, int> helper;        
        for(int i = 0; i < n; i++){
            if(helper.find(intervals[i].start) == helper.end()){
                helper.insert(make_pair(intervals[i].start, i));
            }else{
                // do nothing
            }
        }
        for(int i = 0; i < n; i++){
            ...
            map<int, int>::iterator it = helper.lower_bound(pos);
            if(it == helper.end()){
                res.push_back(-1);
            }else{
