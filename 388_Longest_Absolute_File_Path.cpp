...
public:
	/**
	* Idea: traverse the string
	* we need an array to record the length of each layer
	* 
	*/
    int lengthLongestPath(string input) {
        ...
        int maxLen = 0, ly = 1, ind = 0, flag = 0, cnt = 0;
        vector<int> layer(100, 0);
        for(int i = 0; i < n; i++){
        	while(input[i++] == '\t'){
        		ly++;
        	}
        	...
        	if(flag){
        		maxLen = max(maxLen, layer[ly-1]+cnt);
        	}else{
        		layer[ly] = layer[ly-1] + cnt + 1;
        	}
        	ly = 0;
        	cnt = 0;
        	flag = 0;
