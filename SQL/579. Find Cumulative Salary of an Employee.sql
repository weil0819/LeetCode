...
# METHOD-I: use three Employee relations and exclude the max month one
# Phase-I: comulate three months salary
...
# Phase-II: find the max month for each Id (SHOULE CONSIDER NULL CASE)
...
# Phase-III: merge together, return result
SELECT E1.Id AS Id, E1.Month AS Month, (IFNULL(E1.Salary,0)+IFNULL(E2.Salary,0)+IFNULL(E3.Salary,0)) AS Salary 
FROM (
    SELECT Id,MAX(Month) AS maxmonth 
    FROM Employee 
...
LEFT JOIN Employee E1 ON (tmp.Id=E1.Id AND maxmonth>E1.Month)
LEFT JOIN Employee E2 ON (E1.Id=E2.Id AND E1.Month-1=E2.Month) 
LEFT JOIN Employee E3 ON (E1.Id=E3.Id AND E1.Month-2=E3.Month)
ORDER BY E1.Id, E1.Month DESC;
;
