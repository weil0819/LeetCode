...
public:
	/**
	* Idea: We can use BFS or DFS travese methods to scan the matrix
	* Here, we adopt BFS to do it.
	* 
	*/
    vector<vector<char> > updateBoard(vector<vector<char> >& board, vector<int>& click) {
        ...
        helper.push(click);
        while(!helper.empty()){
        	vector<int> vec = helper.front();
        	helper.pop();
        	if(board[vec[0]][vec[1]] == 'M'){
        		board[vec[0]][vec[1]] = 'X';
        		return board;
        	}
        	if(!isAdjMine(board, vec)){
    ...
    int isAdjMine(vector<vector<char> > board, vector<int> click){
    	...
    	if(x+1 < m && board[x+1][y] == 'X'){
    		return false;
    	}
    	if(x+1 < m && y+1 < n && board[x+1][y+1] == 'X'){
    		return false;
    	}
    	if(y-1 >= 0 && board[x][y-1] == 'X'){
    		return false;
    	}
    	if(y+1 < n && board[x][y+1] == 'X'){
    		return false;
    	}
    	if(x-1 >= 0 && y+1 < n && board[x-1][y+1] == 'X'){
    		return false;
    	}
    	if(x+1 < m && y-1 >= 0 && board[x+1][y-1] == 'X'){
    		return false;
    	}
    }
};
