...
public:
    ...
    MyCircularQueue(int k) {
        Q.resize(k);
        head = 0;           // head starts from 0
        tail = -1;          // tail starts from -1
        len = 0;
    }
    ...
    bool enQueue(int value) {
        if(isFull()) return false;
            ...
            tail = (tail+1)%Q.size();   // Q.size() is aways k
            Q[tail] = value;            // push new value
            len++;                      // increase queue length
            return true;
        }
    }
